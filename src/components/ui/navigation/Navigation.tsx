  ...
  return (
    <nav className="h-16 px-8 py-2 border-2 rounded-2xl border-gray-300 flex justify-between items-center">
      ...
      <ul className="flex justify-between items-center gap-6">
        <NavListItem tooltipText="Characters" href={AppLinks.Characters} text="Characters" />
        {isShowSendMessageLink ? (
          <NavListItem tooltipText="Send message" href={AppLinks.SendMessage} text="Send message" />
        ) : (
        ...
        {isShowAboutPage ? (
          ...
          <NavListItem tooltipText="Profile" href={AppLinks.Profile} text="Profile" />
        )}
      </ul>
    </nav>
  );
